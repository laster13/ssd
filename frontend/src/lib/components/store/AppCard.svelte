<script lang="ts">
	...
	let { app, compact = false, installEnabled = true } = $props<{
		app: App;
		compact?: boolean;
	...
	>
		...
		<div class="relative flex h-full flex-col">
			<div class="flex items-start justify-between gap-4">
				<div
					class="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-[22px] border border-black/5 shadow-[0_6px_18px_rgba(15,23,42,0.06)] dark:border-white/10 dark:shadow-[0_12px_34px_rgba(0,0,0,0.24),inset_0_1px_0_rgba(255,255,255,0.10)]"
					style={`background:${visual.bg}; filter:none;`}
				>
					<div
			...
			<div class="mt-auto flex items-center justify-end gap-3 pt-6">
				...
				{:else}
					<button
						type="button"
						disabled
						class="inline-flex cursor-not-allowed items-center gap-2 rounded-[14px] border border-zinc-300/60 bg-zinc-200/80 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-zinc-500 opacity-80 dark:border-white/10 dark:bg-white/[0.06] dark:text-zinc-500"
