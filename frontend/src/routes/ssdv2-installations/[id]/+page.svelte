...
<svelte:head>
	<title>Suivi installation SSDv2</title>
</svelte:head>
			...
			<div class="relative p-6 sm:p-8">
				...
				<div class="mb-6 rounded-[18px] border border-black/5 bg-white/70 px-4 py-4 text-sm text-zinc-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-zinc-300">
					<div class="grid gap-3 sm:grid-cols-2">
						<div>
							<strong class="text-zinc-900 dark:text-zinc-100">Serveur :</strong>
							<span class="ml-2">{data.machine?.hostname ?? 'Serveur sans nom'}</span>
						</div>

						<div>
							<strong class="text-zinc-900 dark:text-zinc-100">Domaine :</strong>
							<span class="ml-2">{job.payload?.domain ?? '-'}</span>
						</div>

						<div>
							<strong class="text-zinc-900 dark:text-zinc-100">Utilisateur :</strong>
							<span class="ml-2">{job.payload?.username ?? '-'}</span>
						</div>

						<div>
							<strong class="text-zinc-900 dark:text-zinc-100">Email :</strong>
							<span class="ml-2">{job.payload?.email ?? '-'}</span>
						</div>

						<div>
							<strong class="text-zinc-900 dark:text-zinc-100">OAuth :</strong>
							<span class="ml-2">{boolLabel(job.payload?.oauth_enabled)}</span>
						</div>
				...
				{#if job.status === 'completed'}
					<div class="mb-5 rounded-[18px] border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300">
						L’installation SSDv2 est terminée avec succès.
					</div>
				{/if}
